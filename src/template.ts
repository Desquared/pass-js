...
import { ApplePass, Options, PassStyle } from './interfaces';
  ...
  static async load(
    ...
    if (entries.find(entry => entry.isFile() && entry.name === 'pass.json')) {
      ...
      const passJson = JSON.parse(
        (stripJsonComments as any)(jsonContent),
  ...
  static async fromBuffer(
    ...
    for await (const entry of zip) {
      ...
      if (/\/?pass\.json$/i.test(entry.fileName)) {
        ...
        const passJSON = JSON.parse((stripJsonComments as any)(buf.toString('utf8')));
